 ...
 *
 * [변경됨] 실제 API에서는 바디 없이 Header의 X-User-Id(UUID)만으로 요청합니다.
 * 백엔드가 해당 UUID의 현재 복용 제품과 새 제품을 DB에서 조회하여 분석합니다.
 *
 * Mock 모드에서만 아래 형태가 사용됩니다.
 * @typedef {Object} AnalyzeRequest
 * @property {Product[]} [currentProducts]  (Mock 전용) 현재 복용 중인 제품 목록
 * @property {Product}   [newProduct]       (Mock 전용) 새로 구매하려는 제품
 */

/**
 * POST /api/products/current — 복용 중인 제품 추가 요청 바디
 * (Product 형태와 동일하나 id는 서버가 생성)
 * @typedef {Omit<Product, "id">} AddProductRequest
 */

/**
 * GET /api/products/current — 복용 중인 제품 목록 응답
 * @typedef {Object} CurrentProductsResponse
