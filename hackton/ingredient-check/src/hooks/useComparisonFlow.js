 ...
 *
 * [UUID 기반 변경사항]
 * - 첫 마운트 시 서버에서 기존 복용 제품 목록을 불러옵니다.
 * - 제품 추가/삭제 시 서버에도 동기화됩니다.
 * - 분석 요청 시 새 제품을 서버에 저장한 뒤, Header의 X-User-Id 만으로 분석을 요청합니다.
 * - Mock 모드(USE_MOCK=true)에서는 기존과 동일하게 로컬에서 동작합니다.
 *
 * 사용 예시 (화면 쪽 컴포넌트에서):
 *
 ...
 *   flow.currentProducts.isLoading     // 서버에서 목록 불러오는 중 여부
 ...
 *   flow.submitNewProduct()   // 유효성 검사 통과 시 → 새 제품 저장 → 분석 시작 → 3단계 이동
 *
 *   // 3단계 화면
